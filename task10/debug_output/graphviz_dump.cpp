...
void tree_dump_func(const TreeNode_t* node, const char *file, const char *func, int line, const char* debug_msg, ...){
    assert(file);
...
static filenames_for_dump filename_ctor(){
    ...
    char* dot_filename = (char*)calloc(200, sizeof(char));
    if(!dot_filename){
    ...
    if(snprintf(dump.dot_filename, 200, 
                "debug_output/images/dump%d_%04d%02d%02d_%02d%02d%02d.dot", num,
    ...
    if(snprintf(dump.svg_filename, 200, 
                "debug_output/images/dump%d_%04d%02d%02d_%02d%02d%02d.svg", num,
                t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
                t->tm_hour, t->tm_min, t->tm_sec) == -1) {
